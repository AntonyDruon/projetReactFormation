...
const schema = new passwordValidator().is().min(8).is().max(20).has().digits();
...
console.log("req.body", req.body)
  ...
  bcrypt.compare(password, user.password).then((result: boolean) => {
    if (result) {
      console.log("on rentre dans le result");
      const token = sign(
        {
...
router.post("/register", function (req: Request, res: Response) {
  ...
  if (name.length < 3) {
    return res
      .status(400)
      .json({ message: "Le nom doit avoir au moins 3 caractères" });
  }
  ...
  if (!schema.validate(password)) {
    return res.status(400).json({
      message:
        "Le mot de passe doit faire minimum 8 caractères et avoir des chiffres",
    });
  }
  ...
  if (!validator.validate(email)) {
    return res
      .status(400)
      .json({ message: "L'adresse e-mail saisit est invalide" });
  }
  ...
  if (user) {
    if (user.name == name) {
      return res
        .status(400)
        .json({ message: "Le nom d'utilisateur est déjà prit" });
    } else {
